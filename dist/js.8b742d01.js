...
exports.getMousePos = exports.lerp = void 0;
...
var Cursor = /*#__PURE__*/function () {
  function Cursor(el) {
    ...
    var _this$DOM$lines = _slicedToArray(this.DOM.lines, 2);

    this.DOM.lineHorizontal = _this$DOM$lines[0];
    this.DOM.lineVertical = _this$DOM$lines[1];

    // hide initially
    _gsap.gsap.set(this.DOM.lines, {
      opacity: 0
    ...
    this.onMouseMoveEv = function () {
      _this.renderedStyles.tx.previous = _this.renderedStyles.tx.current = mouse.x;
      _this.renderedStyles.ty.previous = _this.renderedStyles.ty.previous = mouse.y;

      _gsap.gsap.to(_this.DOM.lines, {
        duration: 0.9,
        ease: 'Power3.easeOut',
        opacity: 1
      });
    ...
    window.addEventListener('mousemove', this.onMouseMoveEv); // svg filters (ids)
    ...
    this.DOM.feTurbulence = {
      x: document.querySelector("".concat(this.filterId.x, " > feTurbulence")),
      y: document.querySelector("".concat(this.filterId.y, " > feTurbulence"))
  ...
  _createClass(Cursor, [{
    ...
    key: "createNoiseTimeline",
    value: function createNoiseTimeline() {
      var _this2 = this;
      ...
      this.tl = _gsap.gsap.timeline({
        ...
        onStart: function onStart() {
          ...
          _this2.DOM.lineHorizontal.style.filter = "url(".concat(_this2.filterId.x);
          _this2.DOM.lineVertical.style.filter = "url(".concat(_this2.filterId.y);
        },
        onUpdate: function onUpdate() {
          // set the baseFrequency attribute for each line with the current turbulence value
          _this2.DOM.feTurbulence.x.setAttribute('baseFrequency', _this2.primitiveValues.turbulence);

          _this2.DOM.feTurbulence.y.setAttribute('baseFrequency', _this2.primitiveValues.turbulence);
        },
        onComplete: function onComplete() {
          // remove the filters once the animation completes
          _this2.DOM.lineHorizontal.style.filter = _this2.DOM.lineVertical.style.filter = 'none';
        }
    ...
    value: function render() {
      ...
      this.renderedStyles['ty'].current = mouse.y; // use linear interpolation to delay the translation animation
      ...
      _gsap.gsap.set(this.DOM.lineVertical, {
        x: this.renderedStyles['tx'].previous
      });

      _gsap.gsap.set(this.DOM.lineHorizontal, {
...
},{"gsap":"TpQl","./utils":"MgTz"}],"Jy3L":[function(require,module,exports) {
"use strict";
...
var MenuItem = /*#__PURE__*/function () {
  function MenuItem(el) {
    ...
    this.DOM.titleChars = this.DOM.el.querySelectorAll('span.char'); // initial and final colors for each span char (before and after hovering)
  ...
  _createClass(MenuItem, [{
    ...
    value: function onMouseEnter() {
      ...
      this.enterTimeline = _gsap.gsap.timeline({
        defaults: {
          duration: 0.05,
          ease: 'power3',
          x: function x() {
            return _gsap.gsap.utils.random(-15, 15);
          },
          y: function y() {
            return _gsap.gsap.utils.random(-20, 10);
          },
          rotation: function rotation() {
            return _gsap.gsap.utils.random(-5, 5);
          },
          color: function color() {
            return _gsap.gsap.utils.random(0, 3) < 0.5 ? _this2.colors.final : _this2.colors.initial;
          }
        }
      ...
      .to(this.DOM.titleChars, {
        repeat: 3,
        repeatRefresh: true
      }, 0) // reset translation/rotation and set final color
      .to(this.DOM.titleChars, {
        x: 0,
        y: 0,
        rotation: 0,
    ...
    value: function onMouseLeave() {
      ...
      this.leaveTimeline = _gsap.gsap.timeline().to(this.DOM.titleChars, {
        duration: 0.4,
        ease: 'power3',
...
},{"gsap":"TpQl"}],"oR66":[function(require,module,exports) {
...
var splitting = (0, _splitting2.default)(); // initialize custom cursor

var cursor = new _cursor.Cursor(document.querySelector('.cursor')); // Menu Items

_toConsumableArray(document.querySelectorAll('.menu > a')).forEach(function (el) {
  return new _menuItem.MenuItem(el);
...
},{"./cursor":"LMRJ","./menuItem":"Jy3L","splitting/dist/splitting.css":"oR66","splitting/dist/splitting-cells.css":"oR66","splitting":"Qdhu"}]},{},["QvaY"], null)